...
class RemoveTasksByTags(SuiteVisitor):
    ...
    def __init__(self, runid, robotname, mongodb="localhost:27017/", username="", password=""):
        self.runid=runid
        self.robotname=robotname
        # Get tags based on RPA status (executed in other robot run)
        rl = RpaLogger()
        rl.rpa_logger_init(robotname=robotname, runid=runid, mongodbIPPort=mongodb, username=username, password=password)
